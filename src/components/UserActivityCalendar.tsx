    ...
    const { data: activityData } = useQuery({
        ...
        queryFn: async () => {
            ...
            const activityMap = data.reduce((acc: { [key: string]: number }, item) => {
                const date = new Date(item.created_at).toISOString().split('T')[0].replace(/-/g, '/');
    ...
    return (
        <div className="space-y-4">
            <div>
                ...
                <p className="text-gray-600 mb-2">Each lit-up square represents a day you leaped your competition. </p>
            </div>
            <HeatMap
                value={activityData || []}
                width={700}
